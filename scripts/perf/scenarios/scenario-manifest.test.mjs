...
test('the scenario manifest covers every required release scenario', () => {
  ...
  assert.equal(getScenario('artist-dfs-memory').automation, 'self-driven-native')
  assert.ok(getScenario('artist-dfs-memory').durationSeconds >= 600)
  assert.match(getScenario('animation').steps.join(' '), /120 Hz/)
