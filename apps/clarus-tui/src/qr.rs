...
//! modules as cells so a terminal never needs image protocol support.
...
pub enum QrRenderMode {
    ...
    DoubleWidth,
    /// A Unicode half-block combines two vertical modules and is used on narrow terminals.
    HalfBlock,
...
fn module_span(dark: bool) -> Span<'static> {
    Span::styled(
        "  ",
        Style::default().bg(if dark { Color::Black } else { Color::White }),
    )
}
...
fn half_block_span(top_dark: bool, bottom_dark: bool) -> Span<'static> {
    ...
    match (top_dark, bottom_dark) {
        (false, false) => Span::styled(" ", Style::default().bg(white)),
        (true, true) => Span::styled(" ", Style::default().bg(black)),
        (true, false) => Span::styled("▀", Style::default().fg(black).bg(white)),
        (false, true) => Span::styled("▄", Style::default().fg(black).bg(white)),
    }
}
